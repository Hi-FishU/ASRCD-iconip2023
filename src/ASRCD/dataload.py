...
class ASRCDDataSet(Dataset):
    ...
    def __getitem__(self, index):
        split_index = len(self.skills[index]) / 2
        return self.skills[index][:, split_index:], self.skills[
            index][:, :split_index], self.stu_id[index], self.exer_id[
                index], self.answers[index]
...
def dataset_load(graph='Dense',
    ...
    np.save('temp/q_matrix_ext.npy', q_matrix_extend)
    ...
    train_dataset, val_dataset, test_dataset = torch.utils.data.random_split(
        ...
        generator=torch.Generator().manual_seed(4029))
